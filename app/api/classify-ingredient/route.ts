...
//     return NextResponse.json({ message: "DOCUMENT request successful" });
//   } catch (error: any) {
...
//     return NextResponse.json({ message: "DOCUMENT request FAIL" });
//   }
// }
...
export async function GET(req: any, res: any) {
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a food classification and nutrition expert. cooked_yields values can be higher than 1 (e.g. rice and pasta). Provide a JSON response with no extra text, following this structure:\n" +
            "{\n" +
            '  "primary_category": "<string>",\n' +
            '  "secondary_category": "<string>",\n' +
            '  "dietary_classification": "vegan|vegetarian|animal_product|unknown",\n' +
    ...
    const result = JSON.parse(response.choices[0].message.content ?? "{}");
    console.log("result", result);
    return NextResponse.json({ message: "GET request successful", result: result });
  } catch (error: any) {
