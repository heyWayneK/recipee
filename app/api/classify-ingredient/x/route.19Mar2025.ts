...
export async function handler(request: Request) {
  try {
    ...
    if (method === "POST") {
      ...
      const { id, name } = body;
      console.log(`POST request>>>>>>>>: id=${id}, name=${name}`);
      ...
      const ingredient = await prisma.ingredients.upsert({
        ...
        create: {
          ...
          allergy: { connect: { id: 6 } },
          // allergy: jsonData.allergies || "unknown",
      ...
      await prisma.ingredient_cooked_yields.upsert({
      ...
      // NUTRITION  DATA
      // Step 1: Find the latest record for the given ingredient_id
      const latestNutritionId = await prisma.ingredients_nutrition.findFirst({
        where: {
          ingredients_id: ingredient.id,
        },
        orderBy: {
          updated_at: "desc", // Sort by latest createdAt (or use id if autoincrementing)
        },
      });

      const updatedNutrition = await prisma.ingredients_nutrition.upsert({
        where: { id: latestNutritionId?.id || 0 },
        update: {
      ...
      const latestRawToPreppedYields = await prisma.raw_to_prepped_yields.findFirst({
        where: {
          ingredients_id: ingredient.id,
        },
        orderBy: {
      ...
      const latestRawToPreppedYieldsId = latestRawToPreppedYields?.id;

      // Insert data into the raw_to_prepped_yields table
      await prisma.raw_to_prepped_yields.upsert({
        where: { id: latestRawToPreppedYieldsId ?? -1 },
        update: {
          whole: jsonData.raw_to_prepped_yields.whole || 1,
      ...
      return NextResponse.json({ message: `GET NOT SUPPORTED` }, { status: 200 });
    } else {
