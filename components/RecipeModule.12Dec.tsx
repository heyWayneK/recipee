...
import Row_Heading from "./Row_PlatingHeading";
import Row_ElementsList from "./Row_PlatingList";
...
import { calcMarkup, calcProfit, calcXCost, formatWeight, getLiveTotal, replace_ } from "@/lib/utils";
import Table_Cell from "./Table_Cell";
import MenuOption1 from "./MenuOption1";
import MenuOption2 from "./MenuOption2";
import { RecipeModuleProps, data, recipeeUI } from "@/app/data/recipe";
import Row_SubRecipeControls from "./Row_SubRecipeControls";
import Row_SubRecipeSubInfo from "./Row_SubRecipeName";
...
import Row_SubRecipeAddIngredientOrStep from "./Row_SubRecipeAddButtons";
import Row_SubRecipeMethod from "./Row_SubRecipeMethod";
import Row_SubRecipes_ALL from "./Row_SubRecipesAll";
        ...
        >
          {data.uiElements.map((uiElement, rowNum) => {
            ...
            switch (uiName) {
              ...
              case "controls":
                head = (
                  <Table_Cell firstCol={false} header={false} type="controls" iconName="" key={uiName + "_" + rowNum}>
                    {/* SHOW PRICES BUTTON */}
              ...
              case "plating":
                head = (
                  <Table_Cell firstCol={true} header={true} type="plating" iconName="category" key={uiName + "_" + rowNum}>
                    <span>{uiName}</span>
                  </Table_Cell>
                );
                // PLATING QTY
                cells = data.portions.map((portion, col) => (
                  <Table_Cell header={true} type="plating" key={uiName + "_" + rowNum + "_" + col}>
              ...
              case "components":
                ...
                for (i = 0; i < data.components.length; i++) {
                  ...
                  // FIRST COLUMN
                  cells.push(
                    <Table_Cell firstCol={true} rowNum={i} type="component" key={uiName + "_" + rowNum + "_" + i}>
                  ...
                  for (let portionSize of data.portions) {
                    ...
                    cells.push(
                      <Table_Cell type="component" rowNum={i} key={component.name + "_" + uiName + "_" + rowNum + "_" + portionSize} className="">
              ...
              case "ingredient_cost":
                ...
                for (let i = 0; i < data.portions.length; i++) {
                  ...
                  cells.push(
                    <Table_Cell type="sub_total" key={uiName + "_" + rowNum + "_" + portionSize} className="">
                      {data.setting.currency}
                ...
                for (let i = 0; i < data.portions.length; i++) {
                  ...
                  cells.push(
                    <Table_Cell key={uiName + "_" + rowNum + "_" + i} className="">
                      {data.setting.currency}
                      {data.costRules.packagingCosts[data.packagingCostsId[portionSize]].cost.toFixed(2)}
                  ...
                  data.uiElements[rowNum].costsLive[portionSize] = data.costRules.packagingCosts[data.packagingCostsId[portionSize]].cost;
                }
              ...
              case "other_cost":
                ...
                for (let i = 0; i < data.portions.length; i++) {
                  ...
                  cells.push(
                    <Table_Cell key={uiName + "_" + rowNum + "_" + i} className="">
                      {data.setting.currency}
                      {data.costRules.otherCosts[data.otherCostsId[data.portions[i]]].costs.reduce((acc, cost) => (acc = acc + cost.cost), 0).toFixed(2)}

                      {viewPrices && <div className="text-[10px] self-center">Id:{data.otherCostsId[data.portions[i]]}</div>}
                    </Table_Cell>
                  );
                  // LIVE COSTS - OTHER COSTS
                  data.uiElements[rowNum].costsLive[portionSize] = data.costRules.otherCosts[data.otherCostsId[data.portions[i]]].costs.reduce((acc, cost) => (acc = acc + cost.cost), 0);
                }
              ...
              case "markup":
                ...
                for (let i = 0; i < data.portions.length; i++) {
                  ...
                  // PACKAGING COSTS
                  salesPriceExVat += data.costRules.packagingCosts[data.packagingCostsId[portionSize]].cost;
                  // OTHER COSTS
                  salesPriceExVat += data.costRules.otherCosts[data.otherCostsId[portionSize]].costs.reduce((acc, cost) => (acc = acc + cost.cost), 0);

                  const salesPriceWithMarkup = calcProfit(salesPriceExVat, markupRuleType, markupRuleFactor);
                  // OTHER COLUMNS
                  cells.push(
                    <Table_Cell key={uiName + "_" + rowNum + "_" + i} className="flex gap-y-1 flex-col">
                      <div>
                        {data.setting.currency}
              ...
              case "sale_price_(incl_vat)":
                ...
                for (let i = 0; i < data.portions.length; i++) {
                  const portionSize = data.portions[i];
                  const salesPriceIncVat = getLiveTotal(portionSize, "sale_price_(ex_vat)") * (1 + data.setting.vat);
              ...
              case "print":
                ...
                cells = data.portions.map((portion, count) => (
                  <Table_Cell type="print" key={uiName + "_" + rowNum + "_" + count} className="">
                    <div>{<SvgSpriteLink key={`print_${String(portion)}  + "_" + rowNum`} size={30} link={""} iconName="print" className="" />}</div>
                  </Table_Cell>
                ));
                return [...[head], ...cells];
              ...
              case "method":
                head = (
                  <Table_Cell className="flex" firstCol={false} header={false} type="method" iconName="info" key={uiName + "_" + rowNum}>
              ...
              default:
                ...
                // OTHER COLUMNS
                cells = data.portions.map((portion, count) => <Table_Cell key={uiName + "_" + rowNum + "_" + count}>{uiName}</Table_Cell>);
                return [...head, ...cells];
            }
          })}
        </div>
